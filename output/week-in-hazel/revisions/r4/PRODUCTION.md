...
- **Worked example:** [Issue 001, revision 4, illustrated edition](../pdf/week-in-hazel-001-woodcut.pdf). Eleven pages, September 4–10, 2026 UTC. Use its visual restraint, source-linked writing and distinction between landed work and experiments.
- **Exact UI reference:** [matching clean edition](../pdf/week-in-hazel-001.pdf). The same reporting, with authentic lossless PNGs.
- **Preferred screenshot treatment:** [Cards stamp facsimile](assets/probes-cards-woodcut.png), compared with [its original capture](assets/probes-cards-clean.png). Andrew prefers this relatively light stamped quality. The much more heavily woodgrained [RunningSums treatment](assets/modular-focus-woodcut.png) is a contrast example, not the future target. All three existing images remain in this issue by request.
...
Make a small weekly magazine that Hazel contributors will want to read. Aim for roughly ten or eleven pages and 2,800–3,300 extracted words in a busy week, including captions and labels; the useful measure is whether the prose explains concrete consequences. A quieter week earns a shorter issue. Keep a major screenshot when it earns its space, but do not let images turn the reporting into captions. Revision 4 has about 3,000 words in eleven pages. Its modestly reduced Cards plate and compact Modules map leave room for concrete explanation at legible body size.
...
7. Render every page, inspect the result, correct it, validate the final PDF, and update the ledgers. Report completion with the local PDF. Do not publish or message other people unless Andrew authorizes that separately.
...
Use a controlled ragged edge in these narrow columns. Keep ordinary word spacing; do not blanket-justify the body. In the worked PDF, dictionary hyphenation is limited to lowercase alphabetic words of at least eight letters, leaving three letters on each side of a break. Names and code identifiers remain intact. Guard against short final lines and consecutive hyphens, and rewrite an awkward line ending when that reads better. Use about 6.5 pt between real paragraphs, not an entire forced blank line. Headings, captions, and article text have separate spacing rules.

The current renderer is ReportLab and requires Pyphen (the worked revision used 0.18.1). It does not implement CSS `text-wrap: pretty`. For a future HTML edition, use `lang="en-US"`, `hyphens: auto` and `text-wrap: pretty` as progressive enhancements, and consider `text-wrap: balance` for selected short headings. Engine algorithms differ, so proof the actual browser/export. See [PROCESS.md](PROCESS.md) for the comparison, limits and primary documentation. A typography option that works on one paragraph can fail on another.

Do not solve overflow by shrinking all the type. Rebudget a title, compress oversized furniture, move a paragraph, or add a page. Render every page and inspect the actual text. A height check must include subheadings as well as body boxes. Keep paragraph endings and column transitions deliberate; a superficially tidy rectangle is not sufficient.
...
Inspect the generated result against the original. Small glyphs can drift even with this prompt. Retain and link the clean capture, label the plate as a generated facsimile, and never treat its apparent output as new technical evidence. Record the actual prompt, input references, selected output and known limits. Do not regenerate the pilot's other images merely to make them match the chosen exemplar. Whether to use the treatment in a later issue remains an editorial choice; the Cards image defines its preferred strength when used.
