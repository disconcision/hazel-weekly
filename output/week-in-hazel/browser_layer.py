...
class BrowserLayer:
 def __init__(self,root,fonts):
  self.root=root;self.fonts=fonts;self.files=[];self.tmp=root.parents[1]/'tmp/pdfs';self.tmp.mkdir(parents=True,exist_ok=True)
 ...
 def finish(self):
  ...
  for z,job in zip(self.files,jobs):
   ...
   writer.add_metadata({'/Title':f'Week in Hazel | Issue 001 | {z.treatment}', '/Author':'Astra, with Ellis, Rowan and Mica','/Subject':'September 4–10, 2026; production revision 5'})
